...
#### 現在の問題点
- **2つの異なるロギングシステムが混在**
  - `utils.py`の`setup_logger()`（シンプルなログ設定）
  - `logging_config.py`の`setup_plugin_logging()`（構造化ログ設定）
- **各モジュールが独自にロガーを作成**し、一貫性が不足
- **型注釈が曖昧**（`self.logger: Optional[Any] = None`）
- **標準的なPythonロギング作法に準拠していない**

#### 影響範囲
- `src/mkdocs_mermaid_to_image/plugin.py`
- `src/mkdocs_mermaid_to_image/utils.py`
- `src/mkdocs_mermaid_to_image/processor.py`
- `src/mkdocs_mermaid_to_image/markdown_processor.py`  
- `src/mkdocs_mermaid_to_image/image_generator.py`

### 2. その他のコード品質問題

#### 現在の問題点
- **型注釈の不統一**（`Optional[Any]`の多用）
- **設定スキーマの重複定義**（`plugin.py`と`config.py`）
- **複雑なログレベル制御ロジック**
- **モジュール間の循環参照リスク**
- **エラーハンドリングの粒度不足**
...
**作業内容**:
1. `utils.py`の`setup_logger()`を削除
2. `logging_config.py`のロギングシステムに統一
3. モジュールレベルロガーの標準的な実装に変更
...
class MermaidToImagePlugin(BasePlugin[MermaidPluginConfig]):
    def __init__(self) -> None:
        super().__init__()
        # インスタンス変数としてloggerを保持しない
        
    def on_config(self, config: Any) -> Any:
        from .logging_config import setup_plugin_logging
